...
# pandas.io.json._json.FrameWriter._write
try:
    obj = class_constructor()
    ret = obj._write()
    type_pandas_io_json__json_FrameWriter__write = (
    getattr(ret, '__module__', None) + '.' + ret.__class__.__name__
        if getattr(ret, '__module__', None)
        else ret.__class__.__name__
        )
    print("✅ pandas.io.json._json.FrameWriter._write:",
        type_pandas_io_json__json_FrameWriter__write)
except Exception as e:
    type_pandas_io_json__json_FrameWriter__write = '_syft_missing'
    print('❌ pandas.io.json._json.FrameWriter._write: Return unavailable')
    print("  Please fix this return type code until there is no exception")
    print('   Error:', e)
