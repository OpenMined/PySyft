...
class SyftSupport(Extension):
    ...
    def activate(self, actions: List[Action]) -> List[Action]:
        ...
        actions = self.register(actions,change_package_name,after="verify_options_consistency")
...
def change_package_name(struct: Structure, opts: ScaffoldOpts) -> ActionParams:
    ...
    try:
        ...
        raise ImportError("Make support {} is installed and can be imported".format(module))
...
def add_files(struct: Structure, opts: ScaffoldOpts) -> ActionParams:
    ...
    files: Structure = {
        "proto":{
            "sample.proto":(proto_sample,no_overwrite())
        },
        "src": {
            opts["package"]: {
