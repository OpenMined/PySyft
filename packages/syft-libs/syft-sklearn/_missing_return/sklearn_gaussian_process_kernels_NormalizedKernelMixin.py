...
try:
    ...
    ret = obj.diag()
    type_sklearn_gaussian_process_kernels_NormalizedKernelMixin_diag = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.gaussian_process.kernels.NormalizedKernelMixin.diag: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_gaussian_process_kernels_NormalizedKernelMixin_diag = "_syft_missing"
    print(
        "❌ sklearn.gaussian_process.kernels.NormalizedKernelMixin.diag: Return unavailable"
    )  # noqa E501
