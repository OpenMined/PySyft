#!/usr/bin/env python
# coding: utf-8

# ## sklearn.tree._export._MPLTreeExporter

# In[ ]:


# third party
import sklearn


def class_constructor(*args, **kwargs):
    obj = sklearn.tree._export._MPLTreeExporter()
    return obj


# In[ ]:


# sklearn.tree._export._MPLTreeExporter._make_tree
try:
    obj = class_constructor()  # noqa F821
    ret = obj._make_tree()
    type_sklearn_tree__export__MPLTreeExporter__make_tree = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.tree._export._MPLTreeExporter._make_tree: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter__make_tree = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter._make_tree: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.tree._export._MPLTreeExporter.export
try:
    obj = class_constructor()  # noqa F821
    ret = obj.export()
    type_sklearn_tree__export__MPLTreeExporter_export = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.tree._export._MPLTreeExporter.export: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter_export = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter.export: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.tree._export._MPLTreeExporter.get_color
try:
    obj = class_constructor()  # noqa F821
    ret = obj.get_color()
    type_sklearn_tree__export__MPLTreeExporter_get_color = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.tree._export._MPLTreeExporter.get_color: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter_get_color = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter.get_color: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.tree._export._MPLTreeExporter.get_fill_color
try:
    obj = class_constructor()  # noqa F821
    ret = obj.get_fill_color()
    type_sklearn_tree__export__MPLTreeExporter_get_fill_color = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.tree._export._MPLTreeExporter.get_fill_color: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter_get_fill_color = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter.get_fill_color: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.tree._export._MPLTreeExporter.node_to_str
try:
    obj = class_constructor()  # noqa F821
    ret = obj.node_to_str()
    type_sklearn_tree__export__MPLTreeExporter_node_to_str = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.tree._export._MPLTreeExporter.node_to_str: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter_node_to_str = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter.node_to_str: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.tree._export._MPLTreeExporter.recurse
try:
    obj = class_constructor()  # noqa F821
    ret = obj.recurse()
    type_sklearn_tree__export__MPLTreeExporter_recurse = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.tree._export._MPLTreeExporter.recurse: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_tree__export__MPLTreeExporter_recurse = "_syft_missing"
    print(
        "❌ sklearn.tree._export._MPLTreeExporter.recurse: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)
