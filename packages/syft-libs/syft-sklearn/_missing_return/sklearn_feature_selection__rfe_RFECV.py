#!/usr/bin/env python
# coding: utf-8

# ## sklearn.feature_selection._rfe.RFECV

# In[ ]:


# third party
import sklearn


def class_constructor(*args, **kwargs):
    obj = sklearn.feature_selection._rfe.RFECV()
    return obj


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.__getstate__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__getstate__()
    type_sklearn_feature_selection__rfe_RFECV___getstate__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.__getstate__: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV___getstate__ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.__getstate__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.__repr__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__repr__()
    type_sklearn_feature_selection__rfe_RFECV___repr__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.__repr__: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV___repr__ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.__repr__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.__setstate__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__setstate__()
    type_sklearn_feature_selection__rfe_RFECV___setstate__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.__setstate__: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV___setstate__ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.__setstate__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._check_n_features
try:
    obj = class_constructor()  # noqa F821
    ret = obj._check_n_features()
    type_sklearn_feature_selection__rfe_RFECV__check_n_features = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._check_n_features: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__check_n_features = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._check_n_features: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._estimator_type
try:
    obj = class_constructor()
    ret = obj._estimator_type
    type_sklearn_feature_selection__rfe_RFECV__estimator_type = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._estimator_type:", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__estimator_type = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._estimator_type: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._fit
try:
    obj = class_constructor()  # noqa F821
    ret = obj._fit()
    type_sklearn_feature_selection__rfe_RFECV__fit = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV._fit: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__fit = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._fit: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._get_param_names
try:
    obj = class_constructor()  # noqa F821
    ret = obj._get_param_names()
    type_sklearn_feature_selection__rfe_RFECV__get_param_names = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._get_param_names: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__get_param_names = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._get_param_names: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._get_support_mask
try:
    obj = class_constructor()  # noqa F821
    ret = obj._get_support_mask()
    type_sklearn_feature_selection__rfe_RFECV__get_support_mask = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._get_support_mask: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__get_support_mask = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._get_support_mask: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._get_tags
try:
    obj = class_constructor()  # noqa F821
    ret = obj._get_tags()
    type_sklearn_feature_selection__rfe_RFECV__get_tags = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV._get_tags: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__get_tags = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._get_tags: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._more_tags
try:
    obj = class_constructor()  # noqa F821
    ret = obj._more_tags()
    type_sklearn_feature_selection__rfe_RFECV__more_tags = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV._more_tags: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__more_tags = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._more_tags: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._repr_html_
try:
    obj = class_constructor()
    ret = obj._repr_html_
    type_sklearn_feature_selection__rfe_RFECV__repr_html_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV._repr_html_:", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__repr_html_ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._repr_html_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._repr_html_inner
try:
    obj = class_constructor()  # noqa F821
    ret = obj._repr_html_inner()
    type_sklearn_feature_selection__rfe_RFECV__repr_html_inner = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._repr_html_inner: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__repr_html_inner = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._repr_html_inner: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._repr_mimebundle_
try:
    obj = class_constructor()  # noqa F821
    ret = obj._repr_mimebundle_()
    type_sklearn_feature_selection__rfe_RFECV__repr_mimebundle_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._repr_mimebundle_: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__repr_mimebundle_ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._repr_mimebundle_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV._validate_data
try:
    obj = class_constructor()  # noqa F821
    ret = obj._validate_data()
    type_sklearn_feature_selection__rfe_RFECV__validate_data = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV._validate_data: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV__validate_data = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV._validate_data: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.classes_
try:
    obj = class_constructor()
    ret = obj.classes_
    type_sklearn_feature_selection__rfe_RFECV_classes_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.classes_:", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_classes_ = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.classes_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.decision_function
try:
    obj = class_constructor()  # noqa F821
    ret = obj.decision_function()
    type_sklearn_feature_selection__rfe_RFECV_decision_function = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.decision_function: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_decision_function = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.decision_function: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.fit
try:
    obj = class_constructor()  # noqa F821
    ret = obj.fit()
    type_sklearn_feature_selection__rfe_RFECV_fit = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.fit: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_fit = "_syft_missing"
    print("❌ sklearn.feature_selection._rfe.RFECV.fit: Return unavailable")  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.fit_transform
try:
    obj = class_constructor()  # noqa F821
    ret = obj.fit_transform()
    type_sklearn_feature_selection__rfe_RFECV_fit_transform = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.fit_transform: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_fit_transform = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.fit_transform: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.get_params
try:
    obj = class_constructor()  # noqa F821
    ret = obj.get_params()
    type_sklearn_feature_selection__rfe_RFECV_get_params = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.get_params: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_get_params = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.get_params: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.get_support
try:
    obj = class_constructor()  # noqa F821
    ret = obj.get_support()
    type_sklearn_feature_selection__rfe_RFECV_get_support = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.get_support: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_get_support = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.get_support: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.inverse_transform
try:
    obj = class_constructor()  # noqa F821
    ret = obj.inverse_transform()
    type_sklearn_feature_selection__rfe_RFECV_inverse_transform = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.inverse_transform: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_inverse_transform = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.inverse_transform: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.predict
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict()
    type_sklearn_feature_selection__rfe_RFECV_predict = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.predict: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_predict = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.predict: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.predict_log_proba
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict_log_proba()
    type_sklearn_feature_selection__rfe_RFECV_predict_log_proba = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.predict_log_proba: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_predict_log_proba = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.predict_log_proba: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.predict_proba
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict_proba()
    type_sklearn_feature_selection__rfe_RFECV_predict_proba = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.feature_selection._rfe.RFECV.predict_proba: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_predict_proba = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.predict_proba: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.score
try:
    obj = class_constructor()  # noqa F821
    ret = obj.score()
    type_sklearn_feature_selection__rfe_RFECV_score = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.score: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_score = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.score: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.set_params
try:
    obj = class_constructor()  # noqa F821
    ret = obj.set_params()
    type_sklearn_feature_selection__rfe_RFECV_set_params = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.set_params: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_set_params = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.set_params: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.feature_selection._rfe.RFECV.transform
try:
    obj = class_constructor()  # noqa F821
    ret = obj.transform()
    type_sklearn_feature_selection__rfe_RFECV_transform = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.feature_selection._rfe.RFECV.transform: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_feature_selection__rfe_RFECV_transform = "_syft_missing"
    print(
        "❌ sklearn.feature_selection._rfe.RFECV.transform: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)
