...
try:
    ...
    type_sklearn_cluster__birch__CFSubcluster_merge_subcluster = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.cluster._birch._CFSubcluster.merge_subcluster: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_cluster__birch__CFSubcluster_merge_subcluster = "_syft_missing"
    print(
        "❌ sklearn.cluster._birch._CFSubcluster.merge_subcluster: Return unavailable"
    )  # noqa E501
...
try:
    ...
    ret = obj.radius
    type_sklearn_cluster__birch__CFSubcluster_radius = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    ...
    print(
        "❌ sklearn.cluster._birch._CFSubcluster.radius: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)
...
try:
    obj = class_constructor()  # noqa F821
    ret = obj.update()
    type_sklearn_cluster__birch__CFSubcluster_update = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    ...
    print(
        "❌ sklearn.cluster._birch._CFSubcluster.update: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)
