#!/usr/bin/env python
# coding: utf-8

# ## sklearn.naive_bayes._BaseDiscreteNB

# In[ ]:


# third party
import sklearn


def class_constructor(*args, **kwargs):
    obj = sklearn.naive_bayes._BaseDiscreteNB()
    return obj


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.__getstate__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__getstate__()
    type_sklearn_naive_bayes__BaseDiscreteNB___getstate__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB.__getstate__: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB___getstate__ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.__getstate__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.__repr__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__repr__()
    type_sklearn_naive_bayes__BaseDiscreteNB___repr__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.__repr__: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB___repr__ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.__repr__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.__setstate__
try:
    obj = class_constructor()  # noqa F821
    ret = obj.__setstate__()
    type_sklearn_naive_bayes__BaseDiscreteNB___setstate__ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB.__setstate__: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB___setstate__ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.__setstate__: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._check_X
try:
    obj = class_constructor()  # noqa F821
    ret = obj._check_X()
    type_sklearn_naive_bayes__BaseDiscreteNB__check_X = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB._check_X: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__check_X = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._check_X: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._check_X_y
try:
    obj = class_constructor()  # noqa F821
    ret = obj._check_X_y()
    type_sklearn_naive_bayes__BaseDiscreteNB__check_X_y = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB._check_X_y: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__check_X_y = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._check_X_y: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._check_alpha
try:
    obj = class_constructor()  # noqa F821
    ret = obj._check_alpha()
    type_sklearn_naive_bayes__BaseDiscreteNB__check_alpha = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._check_alpha: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__check_alpha = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._check_alpha: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._check_n_features
try:
    obj = class_constructor()  # noqa F821
    ret = obj._check_n_features()
    type_sklearn_naive_bayes__BaseDiscreteNB__check_n_features = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._check_n_features: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__check_n_features = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._check_n_features: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._get_param_names
try:
    obj = class_constructor()  # noqa F821
    ret = obj._get_param_names()
    type_sklearn_naive_bayes__BaseDiscreteNB__get_param_names = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._get_param_names: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__get_param_names = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._get_param_names: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._get_tags
try:
    obj = class_constructor()  # noqa F821
    ret = obj._get_tags()
    type_sklearn_naive_bayes__BaseDiscreteNB__get_tags = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB._get_tags: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__get_tags = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._get_tags: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._init_counters
try:
    obj = class_constructor()  # noqa F821
    ret = obj._init_counters()
    type_sklearn_naive_bayes__BaseDiscreteNB__init_counters = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._init_counters: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__init_counters = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._init_counters: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._joint_log_likelihood
try:
    obj = class_constructor()  # noqa F821
    ret = obj._joint_log_likelihood()
    type_sklearn_naive_bayes__BaseDiscreteNB__joint_log_likelihood = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._joint_log_likelihood: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__joint_log_likelihood = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._joint_log_likelihood: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._more_tags
try:
    obj = class_constructor()  # noqa F821
    ret = obj._more_tags()
    type_sklearn_naive_bayes__BaseDiscreteNB__more_tags = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB._more_tags: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__more_tags = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._more_tags: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._repr_html_
try:
    obj = class_constructor()
    ret = obj._repr_html_
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_html_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB._repr_html_:", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_html_ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._repr_html_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._repr_html_inner
try:
    obj = class_constructor()  # noqa F821
    ret = obj._repr_html_inner()
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_html_inner = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._repr_html_inner: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_html_inner = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._repr_html_inner: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._repr_mimebundle_
try:
    obj = class_constructor()  # noqa F821
    ret = obj._repr_mimebundle_()
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_mimebundle_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._repr_mimebundle_: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__repr_mimebundle_ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._repr_mimebundle_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._update_class_log_prior
try:
    obj = class_constructor()  # noqa F821
    ret = obj._update_class_log_prior()
    type_sklearn_naive_bayes__BaseDiscreteNB__update_class_log_prior = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._update_class_log_prior: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__update_class_log_prior = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._update_class_log_prior: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB._validate_data
try:
    obj = class_constructor()  # noqa F821
    ret = obj._validate_data()
    type_sklearn_naive_bayes__BaseDiscreteNB__validate_data = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB._validate_data: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB__validate_data = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB._validate_data: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.coef_
try:
    obj = class_constructor()
    ret = obj.coef_
    type_sklearn_naive_bayes__BaseDiscreteNB_coef_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.coef_:", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_coef_ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.coef_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.fit
try:
    obj = class_constructor()  # noqa F821
    ret = obj.fit()
    type_sklearn_naive_bayes__BaseDiscreteNB_fit = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.fit: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_fit = "_syft_missing"
    print("❌ sklearn.naive_bayes._BaseDiscreteNB.fit: Return unavailable")  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.get_params
try:
    obj = class_constructor()  # noqa F821
    ret = obj.get_params()
    type_sklearn_naive_bayes__BaseDiscreteNB_get_params = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.get_params: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_get_params = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.get_params: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.intercept_
try:
    obj = class_constructor()
    ret = obj.intercept_
    type_sklearn_naive_bayes__BaseDiscreteNB_intercept_ = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.intercept_:", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_intercept_ = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.intercept_: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.partial_fit
try:
    obj = class_constructor()  # noqa F821
    ret = obj.partial_fit()
    type_sklearn_naive_bayes__BaseDiscreteNB_partial_fit = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.partial_fit: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_partial_fit = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.partial_fit: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.predict
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict()
    type_sklearn_naive_bayes__BaseDiscreteNB_predict = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.predict: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_predict = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.predict: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.predict_log_proba
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict_log_proba()
    type_sklearn_naive_bayes__BaseDiscreteNB_predict_log_proba = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB.predict_log_proba: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_predict_log_proba = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.predict_log_proba: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.predict_proba
try:
    obj = class_constructor()  # noqa F821
    ret = obj.predict_proba()
    type_sklearn_naive_bayes__BaseDiscreteNB_predict_proba = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print(
        "✅ sklearn.naive_bayes._BaseDiscreteNB.predict_proba: ", type(ret)
    )  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_predict_proba = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.predict_proba: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.score
try:
    obj = class_constructor()  # noqa F821
    ret = obj.score()
    type_sklearn_naive_bayes__BaseDiscreteNB_score = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.score: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_score = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.score: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)


# In[ ]:


# sklearn.naive_bayes._BaseDiscreteNB.set_params
try:
    obj = class_constructor()  # noqa F821
    ret = obj.set_params()
    type_sklearn_naive_bayes__BaseDiscreteNB_set_params = (
        getattr(ret, "__module__", "none") + "." + ret.__class__.__name__
    )
    print("✅ sklearn.naive_bayes._BaseDiscreteNB.set_params: ", type(ret))  # noqa E501
except Exception as e:
    type_sklearn_naive_bayes__BaseDiscreteNB_set_params = "_syft_missing"
    print(
        "❌ sklearn.naive_bayes._BaseDiscreteNB.set_params: Return unavailable"
    )  # noqa E501
    print("  Please fix this return type code until there is no exception")
    print("  Error:", e)
