#!/bin/bash
CLEAN="src/syft.xgboost/proto"
PYTHON_OUT="src/syft.xgboost"
PROTO_IN="proto"
...
if [ "$(uname)" == "Darwin" ]; then
    ...
    find src/syft.xgboost/proto -name "*_pb2.py" -print0 | xargs -0 sed -i '' 's/from \(proto.*\) import /from syft.\1 import /g'
else
    echo "Linux"
    find src/syft.xgboost/proto -name "*_pb2.py" -print0 | xargs -0 sed -i 's/from \(proto.*\) import /from syft.\1 import /g'
fi
black src/syft.xgboost/proto
isort src/syft.xgboost/proto
