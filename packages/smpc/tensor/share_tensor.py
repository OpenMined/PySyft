...
class ShareTensor(PassthroughTensor, ShareTensorAncestor):
    def __init__(self, rank, ring_size = 2**64, value = None, seed = None, seed_generators = None):
        ...
        self.generators_przs = [np.random.default_rng(seed) for seed in self.seed_generators]
    ...
    def generate_przs(self, shape):
        ...
        share_1 = self.generators_przs[0].integers(low=self.min_value, high=self.max_value)
        share_2 = self.generators_przs[1].integers(low=self.min_value, high=self.max_value)
