...
from syft import Dataset, Asset
...
class TestMixedInputPolicy:
    ...
    def test_providing_constants_valid(self, submit_code_with_constants_only):
        ...
        assert policy.is_valid(context=None, usr_input_kwargs={'endpoint':'TEST ENDPOINT', 'query':'TEST QUERY'})
    
    def test_constant_vals_can_be_retrieved_by_admin(self, submit_code_with_constants_only):
    ...
    def test_mixed_inputs_valid_with_same_asset(self, worker, ds_client, submit_code_with_mixed_inputs):
        ...
        assert policy.is_valid(context=ds_context, usr_input_kwargs={'data':asset.action_id})


    def test_mixed_inputs_invalid_with_different_asset_raises(self, worker, ds_client, submit_code_with_mixed_inputs):
        
        admin_client = worker.root_client

        ds = Dataset(name="different ds", asset_list=[Asset(name="different asset", data=[1, 2], mock=[2, 3])])
        admin_client.upload_dataset(ds)
        user_code = submit_code_with_mixed_inputs
