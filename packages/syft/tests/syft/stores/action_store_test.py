...
def test_action_store_sanity(store: Any) -> None:
...
def test_action_store_test_permissions(store: Any, permission: Any) -> None:
...
@pytest.mark.flaky(reruns=3, reruns_delay=3)
def test_action_store_test_data_set_get(store: Any) -> None:
    client_key = SyftVerifyKey.from_string(TEST_VERIFY_KEY_STRING_CLIENT)
    root_key = SyftVerifyKey.from_string(TEST_VERIFY_KEY_STRING_ROOT)
