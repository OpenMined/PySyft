...
def patch_protocol_file(filepath: Path) -> None:
...
def ds_client(
    ...
    return guest_client.login(email=guest_email, password=password)
...
@pytest.fixture(autouse=True)
def patched_mongo_client(monkeypatch) -> None:
    monkeypatch.setattr("pymongo.mongo_client.MongoClient", MongoClient)


@pytest.fixture(autouse=True)
def patched_session_cache(monkeypatch) -> None:
    # patching compute heavy hashing to speed up tests

    def _get_key(email, password, connection) -> str:
...
@pytest.fixture(autouse=True)
def patched_user(monkeypatch) -> None:
    # patching compute heavy hashing to speed up tests
