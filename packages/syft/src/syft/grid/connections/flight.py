...
class FlightWebRTCConnection(BidirectionalConnection):
    ...
    async def producer(self) -> None:
        ...
        try:
            while True:
                ...
                import time
                # print('before ser: ', time.time(), msg.message)
                now = time.time()
                data = serialize(msg, to_bytes=True)
                print('producer ser: ', time.time() - now, msg.message)
                data_len = len(data)
                now = time.time()
                # print('before sending: ', time.time(), msg.message)
                ...
                print('producer sending: ', time.time() - now, msg.message)
        except Exception as e:
            traceback_and_raise(e)
    ...
    async def consumer(self, msg: bytes) -> None:
        ...
        try:
            ...
            # print('before deser: ', time.time())
            now = time.time()
            _msg = _deserialize(blob=msg, from_bytes=True)
            print('consumer deser: ', time.time() - now, _msg.message)
