...
class BeaverAction(ImmediateActionWithoutReply):
    ...
    def _object2proto(self) -> BeaverAction_PB:
        values = sy.serialize(self.values)
        locations = sy.serialize(self.locations)
    ...
    def _proto2object(proto: BeaverAction_PB) -> "BeaverAction":
        values = sy.deserialize(blob=proto.values)
        locations = sy.deserialize(blob=proto.locations)
