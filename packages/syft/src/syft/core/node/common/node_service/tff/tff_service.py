...
async def tff_train_federated(
    ...
    async with tff.async_utils.ordered_tasks() as tasks:
        ...
        for round_number in range(start_round, total_rounds + 1):
            # debug(f"Train round {round_number}")
            tasks.add_callable(
                functools.partial(
...
def tff_program(
    ...
    debug("Data sources created")

    # TODO parametrize this
    input_spec = collections.OrderedDict(
            x=tf.TensorSpec(shape=(1,64*64), dtype=tf.float64, name=None),
            y=tf.TensorSpec(shape=(1,1), dtype=tf.float64, name=None),
    )
    ...
    debug('Managers created - training beginning')
        
    asyncio.run(tff_train_federated(
