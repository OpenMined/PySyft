...
from .policy import PolicyCode, SubmitPolicyCode

@serializable(recursive_serde=True)
class PolicyService(AbstractService):
    store: DocumentStore
    policy_code_stash: PolicyStash
    policy_stash: PolicyCodeStash
    
    def __init__(self, store: DocumentStore) -> None:
        self.store = store
        self.policy_stash = PolicyStash(store=store)
        self.policy_code_stash = PolicyCodeStash(store=store)
    ...
    def add_policy_code(
        ...
        result = self.policy_code_stash.set(policy_code.to(PolicyCode, context=context))
        if result.is_err():
            return SyftError(message=str(result.err()))
