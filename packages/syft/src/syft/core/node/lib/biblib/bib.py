...
class Parser:
    ...
    def __init__(self, *, month_style: Optional[str] = "full"):
        ...
        self.__entries: TypeDict = collections.OrderedDict()
    ...
    def get_entries(self) -> TypeDict[Any, Any]:
    ...
    def _fail(self, msg: str, off=None):
        if off is None:
            off = self.__off
    ...
    def _try_tok(self, regexp: str, skip_space: bool = True) -> str:
    ...
    def _scan_balanced_text(self, term) -> str:
        ...
        while self.__off < len(self.__data):
            ...
            if level == 0 and char == term:
                text = self.__data[start : self.__off]  # noqa: 203
                self.__off += 1
                self._skip_space()
                return text
            elif char == "{":
    ...
    def _tok(self, regexp, fail: Optional[str] = None) -> str:
        """Scan token regexp or fail with the given message."""
        res = self._try_tok(regexp)
        if res is None:
            if not fail:
                raise Exception(f"Fail not true. {regexp} {fail}")
            self._fail(fail)
        return res
    ...
    def _scan_command_or_entry(self) -> None:
    ...
    def _scan_field_value(self) -> str:
    ...
    def _scan_field_piece(self) -> str:
...
class FieldError(KeyError):
    def __init__(self, field: str, entry: Optional[Entry] = None):
...
class Entry(collections.OrderedDict):
    ...
    def __init__(
        ...
        pos: Optional[messages.Pos] = None,
        field_pos: TypeDict = {},
    ) -> None:
    ...
    def __getitem__(self, field: Union[str, int, slice]) -> Any:
        try:
            return super().__getitem__(field)
        except KeyError:
            raise FieldError(str(field), self) from None
    ...
    def to_bib(
        self, *, month_to_macro: bool = True, wrap_width: Optional[int] = 70
    ) -> str:
    ...
    def resolve_crossref(self, entries: Entry) -> Entry:
    ...
    def date_key(self) -> TypeTuple:
        ...
        if month is not None:
            ...
            key.append(self.month_num())
        return tuple(key)

    def authors(self, field: str = "author") -> TypeList:
    ...
    def month_num(self, field: str = "month") -> int:
...
def resolve_crossrefs(db: TypeDict, min_crossrefs: Optional[int] = None) -> TypeDict:
        ...
        if crossref is None:
            ...
            with recoverer:
                ...
                if crossref_idx is None:
                    ...
                    counts
                    and min_crossrefs is not None
                    and counts[crossref.lower()] >= min_crossrefs
                ):
                    ndb[key] = entry
                else:
