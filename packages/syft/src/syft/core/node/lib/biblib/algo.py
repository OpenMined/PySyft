...
class NameParser:
    ...
    def __depth(self, data: TypeList) -> TypeList:
    ...
    def __split_depth0(self, regexp: str, data: TypeList, flags: int = 0):
        regexp = re.compile(regexp, flags=flags)
        depths = self.__depth(data)
        parts, last = [], 0
        for m in regexp.finditer(data):
            if depths[m.start()] == 0:
                parts.append(data[last : m.start()])  # noqa: 203
                last = m.end()
                if regexp.groups:
    ...
    def _first_char(self, data: TypeList) -> str:
    ...
    def __split_von_last(self, toks: TypeList) -> TypeTuple[TypeList, TypeList]:
    ...
    def parse(self, string: str, pos: messages.Pos) -> TypeList:
        ...
        for name_string in name_strings:
            ...
            if "," in name_trailing[len(name_string) :]:  # noqa: 203
                # BibTeX warns about this because it often indicates a
                # bigger syntax problem
                pos.warn(f"trailing comma after name `{name_string}'")
            ...
            if len(parts) == 1:
                ...
                pos.raise_error(f"too many commas in name `{name_string}'")
...
class Name(collections.namedtuple("Name", "first von last jr")):
    ...
    def pretty(self, template: str = "{first} {von} {last} {jr}"):
        ...
        def priority(string: Optional[str]):
            if not string:
                return 0
            elif string.isspace():
                return 1
        ...
        pieces = pieces[first_field : last_field + 1]  # noqa: 203
        return leading + "".join(pieces) + trailing


def parse_names(string: str, pos: messages.Pos = messages.Pos.unknown) -> TypeList:
...
def parse_month(string: str, pos: messages.Pos = messages.Pos.unknown) -> int:
    ...
    for i, name in enumerate(_MONTHS):
        if name.startswith(val) and len(val) >= 3:
            return i + 1
    pos.raise_error(f"invalid month `{string}'")
...
class TeXProcessor:
    ...
    def process(self, string, pos):
        ...
        while True:
            ...
            if nval is None:
                if macro.startswith("\\"):
                    pos.raise_error(f"unknown macro `{macro}'")
                pos.raise_error(f"unknown special character `{macro}'")
            self.__data = (
                self.__data[: m.start()] + nval + self.__data[self.__off :]  # noqa: 203
            )  # noqa: 203
        ...
        if self.__data[self.__off] == "{":
            ...
            while depth or self.__off == start:
                if self.__data[self.__off] == "{":
                    ...
                    depth -= 1
                self.__off += 1
            return self.__data[start + 1 : self.__off - 1]  # noqa: 203
        elif self.__data[self.__off] == "\\":
