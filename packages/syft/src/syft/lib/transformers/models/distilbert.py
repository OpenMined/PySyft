...
class Embeddings(SyModule):
    ...
    def forward(self, input_ids: torch.LongTensor) -> torch.FloatTensor:
        ...
        position_ids = remote_torch.unsqueeze(position_ids, 0).expand(input_ids.size(0), input_ids.size(1))  # (bs, max_seq_length)
...
class MultiHeadSelfAttention(SyModule):
    ...
    def unshape(self, x, bs, dim_per_head):
        """group heads"""
        return x.transpose(1, 2).contiguous().view(bs, -1,
...
class SyDistilBert(SyModule):
    def __init__(self, config: DistilBertConfig, **kwargs):
        ...
        # Transformer
        transformer_x = torch.rand(
            *kwargs['inputs']['input_ids'].size(), config.dim)
