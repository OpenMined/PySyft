        ...
        if canonical_name != "" and canonical_name is not None:
            ...
            try:
                ...
                if is_syftobject:
                    ...
                    try:
                        version_mutations = dp.protocol_history[SYFT_086_PROTOCOL_VERSION]["object_versions"][canonical_name]
                    except Exception:
                        print(f"could not find {canonical_name} in protocol history")
                        raise

                    version_086 = max([int(k) for k, v in version_mutations.items() if v["action"] == "add"])
                    try:
                        res = cls.__object_serialization_registry__[canonical_name, version_086]
                    except Exception:
        ...
        for type_from_mro in type_from.mro():
            ...
            for type_to_mro in type_to.mro():
                ...
                if mapping_string in SyftObjectRegistry.__object_transform_registry__:
                    return SyftObjectRegistry.__object_transform_registry__[mapping_string]
        raise Exception(
