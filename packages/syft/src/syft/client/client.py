...
def forward_message_to_proxy(
    ...
    return debox_signed_syftapicall_response(signed_result)
...
class HTTPConnection(ServerConnection):
    ...
    def _make_get(
        ...
        if response.status_code != 200:
            msg = f"Failed to fetch {url}. Response returned with code {response.status_code}"
    ...
    def get_server_metadata(
        ...
        if self.proxy_target_uid:
            return forward_message_to_proxy(
    ...
    def make_call(self, signed_call: SignedSyftAPICall) -> Any | SyftError:
        ...
        if response.status_code != 200:
            msg = f"Failed to fetch metadata. Response returned with code {response.status_code}"
            raise requests.ConnectionError(
                msg,
            )

        return _deserialize(response.content, from_bytes=True)
...
class SyftClient:
    ...
    def _get_communication_protocol(
        ...
        if len(common_protocols) == 0:
            msg = "No common communication protocol found between the client and the server."
            raise Exception(
                msg,
            )
    ...
    def verify_key(self) -> SyftVerifyKey:
        if self.credentials is None:
            msg = "SigningKey not set on client"
    ...
    def exchange_route(
        ...
        if protocol == SyftProtocol.HTTP:
            ...
            msg = f"Invalid Route Exchange SyftProtocol: {protocol}.Supported protocols are {SyftProtocol.all()}"
            raise ValueError(
                msg,
            )
    ...
    def _fetch_api(self, credentials: SyftSigningKey) -> SyftAPI:
        ...
        if self.credentials is None:
            msg = f"{self}'s credentials (signing key) is None!"
            raise ValueError(msg)
