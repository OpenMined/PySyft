...
class RemoteFunction(SyftObject):
    ...
    def function_call(
        ...
        if "blocking" in self.signature.parameters:
            msg = f"Signature {self.signature} can't have 'blocking' kwarg because it's reserved"
            raise Exception(
                msg,
            )
        ...
        if path in API_CALLS_THAT_REQUIRE_REFRESH and self.refresh_api_callback is not None:
...
def generate_remote_function(
    ...
    if "blocking" in signature.parameters:
        msg = f"Signature {signature} can't have 'blocking' kwarg because it's reserved"
        raise Exception(
            msg,
        )
...
def generate_remote_lib_function(
    ...
    if "blocking" in signature.parameters:
        msg = f"Signature {signature} can't have 'blocking' kwarg because its reserved"
        raise Exception(
            msg,
        )
...
def result_needs_api_update(api_call_result: Any) -> bool:
    ...
    if isinstance(api_call_result, Request) and any(
        ...
        return True
    return bool(isinstance(api_call_result, SyftSuccess) and api_call_result.require_api_update)
...
class SyftAPI(SyftObject):
    ...
    def update_api(self, api_call_result: Any) -> None:
        ...
        if result_needs_api_update(api_call_result) and self.refresh_api_callback is not None:
            self.refresh_api_callback()
...
class ServerIdentity(Identity):
    ...
    def from_api(api: SyftAPI) -> ServerIdentity:
        ...
        if api.connection is None:
            msg = "{api}'s connection is None. Can't get the server identity"
            raise ValueError(
                msg,
            )
