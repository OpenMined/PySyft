...
def create_deployment(client: OblvClient, domain_clients: list, deployment_name: Optional[str] = None, key_name: str = "", connection_port: int = 3032) -> str:
    ...
    result = DeploymentClient(deployment_id=res.deployment_id, oblv_client = client, domain_clients=domain_clients,user_key_name=key_name, connection_port=connection_port)
    return result
