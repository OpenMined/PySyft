...
class DeploymentClient():
    ...
    def __init__(self,deployment_id: str=None, oblv_client: OblvClient = None,domain_clients: List[Any] = [],user_key_name=""):
    ...
    def request_publish(self, dataset_id, sigma = 0.5):
        ...
        if depl.is_deleted==True:
            raise Exception("User cannot connect to this deployment, as it is no longer available.")
        req = requests.post(self.__conn_string + "/tensor/publish/request", json={"dataset_id": dataset_id, "sigma": sigma})
        if req.status_code==401:
            raise OblvUnAuthorizedError()
        elif req.status_code == 400:
            raise OblvEnclaveError(req.json()["detail"])
            ...
            for o in self.client:
                o.oblv.publish_budget(deployment_id=self.deployment_id,publish_request_id=data,client=self.oblv_client)
            return data
    ...
    def check_publish_request_status(self,publish_request_id):
        ...
        if depl.is_deleted==True:
            raise Exception("User cannot connect to this deployment, as it is no longer available.")
        req = requests.get(self.__conn_string + "/tensor/publish/result_ready?publish_request_id="+publish_request_id)
    ...
    def fetch_result(self, publish_request_id):
        ...
        if depl.is_deleted==True:
            raise Exception("User cannot connect to this deployment, as it is no longer available.")
        req = requests.get(self.__conn_string + "/tensor/publish/result?request_id="+publish_request_id)
        if req.status_code==401:
            raise OblvUnAuthorizedError()
        elif req.status_code == 400:
            raise OblvEnclaveError(req.json()["detail"])
    ...
    def fetch_current_proxy_logs(self,follow=False, tail=False):
        ...
        """
        if self.__logs==None:
            print(ENCODE_RED+ENCODE_BOLD+"Exception"+ENCODE_BLACK+ENCODE_NO_STYLE+": Logs not initiated",file=sys.stderr)
