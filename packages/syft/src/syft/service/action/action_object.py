...
class Action(SyftObject):
    ...
    Attributes:
        path (str | None): The path of the Type of the remote object.
        op (str | None): The method to be executed from the remote object.
        remote_self (LineageID | None): The extended UID of the SyftObject.
        args (list[LineageID]): Arguments passed to the operation.
        kwargs (dict[str, LineageID]): Keyword arguments passed to the operation.
        result_id (LineageID): Extended UID of the resulted SyftObject.
        action_type (ActionType | None): The type of action being performed.
        create_object (SyftObject | None): The Syft object to be created.
        user_code_id (UID | None): The UID associated with the user code.
    """
...
class PreHookContext(SyftBaseObject):
    ...
    Attributes:
        obj (Any): The ActionObject to use for the action.
        op_name (str): The method name to use for the action.
        server_uid (Optional[UID]): Optional Syft server UID.
        result_id (Optional[Union[UID, LineageID]]): Optional result Syft UID.
        action (Optional[Action]): The action generated by the current hook.
        result_twin_type (TwinMode | None): The twin mode for the result.
        action_type (ActionType | None): The type of action being performed.
    """
...
def make_action_side_effect(
    ...
    Args:
        context (PreHookContext): The PreHookContext object.
...
def send_action_side_effect(
    ...
    Raises:
        RuntimeError: If the action cannot be created or if an unexpected response is received.
    """
    try:
...
def propagate_server_uid(
    ...
    Args:
        context (PreHookContext): The PreHookContext object.
        op (str): Which operation was executed.
    ...
    Raises:
        RuntimeError: If the parent object does not have a syft_server_uid or
                     if the output is not wrapped and should not propagate the server_uid.
    """
    ...
    try:
        ...
        if op not in context.obj._syft_dont_wrap_attrs():
            ...
            raise RuntimeError(
                "Don't propagate server_uid because output isn't wrapped"
            )
    except Exception:
...
class ActionObject(SyncableSyftObject):
    ...
    def syft_execute_action(
        ...
        Args:
            action (Action): Which action to execute.
            sync (bool): Run sync/async.

        Returns:
            ActionObjectPointer: The pointer to the action object.

        Raises:
            SyftException: If the pointer doesn't have a server_uid.
            ValueError: If the API is not found.
        """
    ...
    def syft_make_action(
        ...
        Args:
            path (str): The path of the Type of the remote object.
        ...
        Returns:
            Action: The generated action object.
        """
        if args is None:
            args = []
        if kwargs is None:
    ...
    def syft_remote_method(
        ...
        Returns:
            Callable: A function to perform the operation.
        """
    ...
    def get(self, block: bool = False) -> Any:
        ...
        Args:
            block (bool): Whether to block until the object is available.

        Returns:
            Any: The object retrieved from the Syft Client.
        """
        # relative
    ...
    def from_obj(
        ...
        Args:
            syft_action_data (Any): The object to be converted to a Syft ActionObject.
            id (UID | None): Which ID to use for the ActionObject. Optional.
            syft_lineage_id (LineageID | None): Which LineageID to use for the ActionObject. Optional.
            syft_client_verify_key (SyftVerifyKey | None): The client verification key.
            syft_server_location (UID | None): The server location UID.
            syft_resolved (bool | None): Whether the object is resolved.
            data_server_id (UID | None): The data server ID.
            syft_blob_storage_entry_id (UID | None): The blob storage entry ID.

        Returns:
            ActionObject: The created ActionObject.

        Raises:
            SyftException: If the object's type is unsupported.
            ValueError: If the UID and LineageID don't match.
        """
        if id is not None and syft_lineage_id is not None and id != syft_lineage_id.id:
            raise ValueError("UID and LineageID should match")
    ...
    def empty(
        ...
        """Create an ActionObject from a type, using an ActionDataEmpty object.

        Args:
            syft_internal_type (type[Any] | None): The Type for which to create an ActionDataEmpty object.
            id (UID | None): Which ID to use for the ActionObject.
            syft_lineage_id (LineageID | None): Which LineageID to use for the ActionObject.
            syft_resolved (bool | None): Whether the object is resolved.
            data_server_id (UID | None): The data server ID.
            syft_blob_storage_entry_id (UID | None): The blob storage entry ID.

        Returns:
            Self: The created ActionObject.
        """
        syft_internal_type = (
