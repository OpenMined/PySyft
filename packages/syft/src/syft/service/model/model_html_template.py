...
header_line_html = (
    ...
    + type_html
    + """
        <span class='jobs-title'>${model_name}</span>
        </div>
        ${button_html}
    </div>
    <div style="height:16px;"></div>
...
def generate_attr_html(attrs: Dict[str, str]):
    ...
    for key in attrs:
        ...
        <span style="font-weight: 700; line-weight: 19.6px; font-size: 14px; font: 'Open Sans'">{key}:</span>
            {attrs[key]}
        </div>
        """
...
assets_html = """<div id="${result_tab_id}" class="tab-${identifier}" style="background: #F4F3F6;
...
model_repr_template = (
    f"""
<!-- Start model_repr_template -->
<div class="syft_widget">
