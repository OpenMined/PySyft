...
class UserCodeService(AbstractService):
    ...
    def _submit(
        ...
        return self.stash.set(context.credentials, code)
    ...
    def _get_or_submit_user_code(
        ...
        if isinstance(code, UserCode):
            ...
            return self._submit(context, code, exists_ok=True)  # type: ignore
    ...
    def request_code_execution(
        ...
        return self._request_code_execution(
            context,
            user_code,
            reason,
        )
    ...
    def has_code_permission(
        ...
        if context.credentials not in (context.server.verify_key, code_item.user_verify_key):
            return SyftError(
