...
cases(
  ...
  ({params, labelClasses, iconClasses}) => {
    render(
      <Checkbox labelProps={{'data-testid': 'label-testid'}} iconProps={{'data-testid': 'icon-testid'}} {...params} />
    )
  ...
  [
    ...
    },
    {
      ...
      params: {checked: true, onChange: jest.fn},
      labelClasses: 'dark:text-gray-200',
      iconClasses: 'border-2 dark:bg-primary-400 dark:border-primary-400 dark:text-gray-900'
    },
    {
    ...
    },
    {
      ...
      params: {disabled: true},
      labelClasses: 'text-opacity-40',
      iconClasses: 'border-2 dark:bg-primary-800 dark:border-primary-800 dark:text-gray-900'
    }
  ]
)
...
describe('accessibility', () => {
  ...
  test('component is accesible when aria-label is passed', async () => {
    const {container} = render(<Checkbox aria-label="Label" data-testid="test-id" />)
    expect(await axe(container)).toHaveNoViolations()
  })

  test("aria checked equals false when isn't checked or indeterminate", async () => {
    const {container} = render(<Checkbox data-testid="test-id" label="Label" />)
  ...
  test('aria checked equals true when is checked', async () => {
    const {container} = render(<Checkbox checked data-testid="test-id" onChange={jest.fn} label="Label" />)
  ...
  test('aria checked equals mixed when is indeterminate', async () => {
    const {container} = render(<Checkbox indeterminate onChange={jest.fn} data-testid="test-id" label="Label" />)
    const input = screen.getByTestId('test-id')
