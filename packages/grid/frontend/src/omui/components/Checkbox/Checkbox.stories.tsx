...
const cases: Partial<CheckboxProps>[] = [{}, {checked: true}, {indeterminate: true}]

const AllCases: Story<CheckboxProps> = () => {
  const [values, setValues] = useState<{id: string | number; checked: boolean}[]>(
  ...
  function handleField(e: any, item: any) {
    setValues(old => old.map(i => (i.id === item.id ? {...item, checked: e.target.checked} : i)))
  }
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="flex space-x-12 p-4 rounded-md max-w-min border border-dashed border-purple-400">
          {states.map((stateProps, stateIdx) => (
            <div className="space-y-2" key={stateIdx}>
              {cases.map((props, idx) => (
                <Checkbox key={idx} {...stateProps} {...props} onChange={() => void 0} label="Label" />
              ))}
            </div>
          ))}
        </div>
      ...
      <div className="space-y-1">
        <Checkbox indeterminate={someChecked} checked={allChecked} onChange={handleToggleAll} label="All fields" />
        {values.map(item => (
          <div className="pl-4 space-y-2" key={item.id}>
            <Checkbox checked={item.checked} onChange={e => handleField(e, item)} label="Label" />
          </div>
        ))}
      </div>
    </div>
...
export const Template: Story<CheckboxProps> = args => {
  ...
  return (
    <>
      ...
      <Checkbox {...args} indeterminate={checked} onChange={e => setChecked(e.target.checked)} />
    </>
  )
}
