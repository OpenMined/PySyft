...
import type {ButtonHTMLAttributes, ElementType, PropsWithChildren} from 'react'
 ...
 */
const buttonBorderlessSize: iconSizeRecord<string> = {xs: 'p-2', sm: 'p-2', md: 'px-3 py-2', lg: 'px-4 py-3'}
const buttonBorderSizes: iconSizeRecord<string> = {xs: 'p-1.5', sm: 'p-1.5', md: 'px-2.5 py-1.5', lg: 'px-3.5 py-2.5'}
const iconSizes: iconSizeRecord<IconSizeProp> = {xs: 'md', sm: 'md', md: 'lg', lg: 'xl'}
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  {size = 'md', variant = 'primary', color = 'primary', disabled, leftIcon, rightIcon, className, children, ...props},
  ref
) {
  ...
  return (
    <button className={buttonClasses} ref={ref} disabled={disabled} {...props}>
      {leftIcon && <Icon size={iconSizes[size]} variant="ghost" icon={leftIcon} containerProps={{className: 'pr-2'}} />}
      ...
      {rightIcon && (
        <Icon size={iconSizes[size]} icon={rightIcon} variant="ghost" containerProps={{className: 'ml-2'}} />
      )}
    </button>
  )
})
...
export type IconButtonProps = Exclude<ButtonProps, 'rightIcon' | 'leftIcon' | 'children' | 'variant'> & {
  icon: ElementType
  variant?: IconButtonVariant
  rounded?: boolean
}

const iconBorderlessSize: iconSizeRecord<string> = {xs: 'p-2', sm: 'p-2.5', md: 'p-3', lg: 'p-3.5'}
const iconBorderSizes: iconSizeRecord<string> = {xs: 'p-1.5', sm: 'p-2', md: 'p-2.5', lg: 'p-3'}

const IconButton = forwardRef<HTMLButtonElement, IconButtonProps>(function IconButton(
  {size = 'md', variant = 'primary', disabled, icon, rounded, className, ...props},
  ref
) {
  const hasBorder = variant === 'outline'
  ...
  return (
    <button className={iconButtonClasses} disabled={disabled} ref={ref} {...props}>
