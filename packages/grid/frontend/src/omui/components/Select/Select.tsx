...
function Option({ option, onClick }: OptionProps) {
  ...
  const noActiveValue =
    (value === undefined || value === null) && option.placeholder
  ...
  const handleKeyDown = (
    ...
    const eventCases: EventCases = {
      [Keys.ArrowDown]: () => {
        const nextSibling =
          document.getElementById(optionId)?.nextElementSibling?.id
        if (nextSibling) return onKeyboardNavigate?.(nextSibling)
      },
      [Keys.ArrowUp]: () => {
        const prevSibling =
          document.getElementById(optionId)?.previousElementSibling?.id
