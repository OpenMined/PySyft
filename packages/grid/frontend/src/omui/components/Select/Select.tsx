import React, {createContext, forwardRef, useCallback, useContext, useLayoutEffect, useRef, useState} from 'react'
...
import type {PropsWithRef, KeyboardEvent as ReactKeyboardEvent, MouseEvent as ReactMouseEvent} from 'react'
...
type SelectContextValues = {value?: OptionValue; closeMenu?: () => void; onKeyboardNavigate?: (id: string) => void}
...
const Select = forwardRef<HTMLDivElement, SelectProps>(function Select(
  {size = 'md', placeholder = 'Select option', label, required, disabled, value, onChange, options, error, ...props},
  ref
) {
  ...
  const activeLabel = value !== undefined || value !== null ? options?.find(i => i.value === value)?.label : null
  ...
  return (
    <SelectContext.Provider value={{value, closeMenu: handleCloseMenu, onKeyboardNavigate: handleKeyboardNavigate}}>
        ...
        <div
          ...
          {isOpen && options?.length ? (
            <ul id="omui-menu-list" role="menu" className={optionsMenuClasses}>
              <Option option={{label: placeholder, value: '', placeholder: true, disabled: true}} />
              {options.map(option => (
