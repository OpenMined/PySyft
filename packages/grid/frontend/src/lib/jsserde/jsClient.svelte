    ...
    export class JSClient {
        ...
        constructor(url){
            return ( async () => {
                    ...
                    this.node_id = await this.metadata.then(metadata => {return metadata.get('id').get('value')})
                    return this
                })();
            }
        ...
        get user(){
            if (!this.access_token) {
		        ...
		        }).then(response => response.json()).then(body => {return body})
            }
        }
        ...
        createUser(parameters) {
            return this.send(parameters, 'syft.core.node.common.node_service.user_manager.new_user_messages.CreateUserMessage')
        }

        updateUser(parameters) {
            return this.send(parameters, 'syft.core.node.common.node_service.user_manager.new_user_messages.UpdateUserMessage')
        }

        updateConfigs(parameters){
            return this.send(parameters,'syft.core.node.common.node_service.node_setup.node_setup_messages.UpdateSetupMessage')
        }
        ...
        send(parameters, fqn) {
                ...
                return fetch(this.msg_url, {
                    ...
                    .then(response => {return this.serde.deserialize(response)})
            }
        }
