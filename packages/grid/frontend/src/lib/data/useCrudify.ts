...
import type {UseQueryOptions, QueryFunction, MutationFunction, UseMutationOptions} from 'react-query'
...
function useCrudifyRQ<T>({
  ...
  const useOne = (id: string | number, config?: UseQueryOptions<T>) =>
    useQuery<T, ErrorMessage>([id, ...queryKeys], fetchOneFn, {...config, enabled: !!id})

  const useCreate = <U>(config: UseMutationOptions<T, ErrorMessage, U> = {}, options?: AxiosRequestConfig) =>
    useMutation<T, ErrorMessage, U>(data => createFn({data, queryKeys, options}), {
      ...config,
      onSuccess: (...args) => {
  ...
  const useUpdate = <U>(id: string | number, config: UseMutationOptions<T, ErrorMessage, U> = {}) =>
    useMutation<T, ErrorMessage, U>(data => updateFn({data, id, queryKeys}), {
      ...config,
      onSuccess: (...args) => {
  ...
  const useDelete = <U>(id: string | number, config: UseMutationOptions<T, ErrorMessage, void, U> = {}) =>
