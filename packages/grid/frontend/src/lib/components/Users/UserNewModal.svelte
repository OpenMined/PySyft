<script lang="ts">
  import ButtonGhost from '../ButtonGhost.svelte';
  import CalloutCard from '../CalloutCard.svelte';
  import XIcon from '../icons/XIcon.svelte';
  import Modal from '../Modal.svelte';

  export let onClose: () => void;
  export let onCreateGeneralUser: () => void;
</script>

<div class="absolute top-0 left-0 w-screen h-screen bg-gray-800 bg-opacity-50">
  <div class="h-screen w-screen flex items-center justify-center">
    <div class="desktop:w-[662px]">
      <Modal>
        <div slot="header" class="flex w-full">
          <h3 class="text-2xl capitalize font-regular w-full">Create a user account</h3>
          <button class="self-start" on:click={onClose}>
            <XIcon class="w-6 h-6" />
          </button>
        </div>
        <div slot="body" class="flex w-full justify-center py-2">
          <CalloutCard on:click={onCreateGeneralUser}>
            <img
              src="images/illustrations/create-general-user.png"
              alt="Create general user alt"
              class="object-none object-center"
              slot="image"
            />
            <span slot="title">Create general user</span>
            <span slot="description">
              Create accounts for outside researchers to safely study from your data without needing
              a copy or direct access.
            </span>
          </CalloutCard>
        </div>
        <div class="flex w-full justify-end" slot="button-group">
          <ButtonGhost variant="gray" on:click={onClose}>Cancel</ButtonGhost>
        </div>
      </Modal>
    </div>
  </div>
</div>
