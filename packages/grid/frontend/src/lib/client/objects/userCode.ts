...
class InputPolicy {
  ...
  constructor(id: UUID, inputs: InputObject) {
    this.id = id;
    this.inputs = new Map(Object.entries(inputs).map(([key, value]) => [JSON.parse(key), value]));
  }
}
...
class OutputPolicy {
   ...
   */
  constructor(id: UUID, outputs: any[], state_type: string, output_state: object) {
    this.id = id;
...
export class UserCode {
  ...
  constructor(obj: {
    ...
    this.id = obj.id;
    this.input_policy = new InputPolicy(obj.input_policy.id, obj.input_policy.inputs);
    ...
    const newStatus = new Map(
      Object.entries(obj.status.base_dict).map(([key, value]) => [JSON.parse(key), value])
    );
    this.status = newStatus;
    this.signature = new Signature(obj.signature.parameters, obj.signature.return_annotation);
  }
}
