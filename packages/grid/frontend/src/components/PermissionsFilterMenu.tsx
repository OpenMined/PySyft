...
export function PermissionsFilter({setSelectedFilters}: Filter) {
  const [selectedPermissions, setSelected] = useState<boolean[]>(Object.keys(gridPermissions).map(() => false))
  const totalSelected = useMemo(() => selectedPermissions.filter(item => item).length, [selectedPermissions])
