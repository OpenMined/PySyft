    ...
    <List>
      {users.map(({ user, role, permissions }: EnhancedUser) => (
        <UserListContext.Provider
          value={{ user, role, permissions, me }}
          key={`list-${user.email}`}
        >
          <AccordionListItem>
            <UserInfoTitle />
  ...
  return (
    <div className="max-w-xl">
      ...
      <div className="space-y-2">
        <p>
          Permissions are set by roles and they are used for managing the
          domain.
        </p>
        <p>
          For example, a Data Scientist is not expected to have any managerial
          permissions such as editing users or uploading datasets, while a Data
          Compliance Office should have at least the permission to triage
          requests. A Data Owner has all permissions.
        </p>
        <p>
            ...
            {permissions[permission] ? (
              <>
                ...
                <p className="italic font-medium">
                  {gridPermissions[permission]?.name}
                </p>
              </>
            ) : (
              <>
...
function ChangeRole() {
  ...
  const mutation = useMutation(
    () => api.patch(`${cacheKeys.users}/${user.id}`, { role: newRole }),
    {
...
function ChangePassword() {
  ...
  const invalidate = () => queryClient.invalidateQueries(cacheKeys.users)
  const mutation = useMutation(
    () => api.patch(`${cacheKeys.users}/${user.id}`, { password }),
    {
      onSuccess: invalidate,
    }
  )
...
function ChangeEmail() {
  ...
  const invalidate = () => queryClient.invalidateQueries(cacheKeys.users)
  const mutation = useMutation(
    () => api.patch(`${cacheKeys.users}/${user.id}`, { email }),
    { onSuccess: invalidate }
  )
...
function ChangeEpsilon() {
  ...
  const mutation = useMutation(
    () =>
      api.patch(`${cacheKeys.users}/${user.id}`, {
...
function DeleteUser() {
  ...
  const mutation = useMutation(
    () => api.delete(`${cacheKeys.users}/${user.id}`),
    { onSuccess: invalidate }
  )
...
export function UserCreate({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation(
    (user: UserSignUp) => api.post<User>(cacheKeys.users, user),
    {
      onSuccess: () => {
        invalidate()
        reset()
  ...
  return (
    <div className="p-8 space-y-6 rounded-md bg-blueGray-200">
      <header className="max-w-xl space-y-2">
        ...
        <p>
          PyGrid utilizes users and roles to appropriately permission data at a
          higher level. All users with the permission{' '}
          <span className="p-1 text-xs uppercase bg-gray-100 text-trueGray-800 tracker-tighter">
            Can create users
          </span>{' '}
