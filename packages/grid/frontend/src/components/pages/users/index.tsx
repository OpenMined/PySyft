  ...
  return (
    <div className="max-w-xl">
      ...
      <div className="space-y-2">
        ...
        <p>
          For example, a Data Scientist is not expected to have any managerial permissions such as
          editing users or uploading datasets, while a Data Compliance Office should have at least
          the permission to triage requests. A Data Owner has all permissions.
        </p>
        <p>
...
function ChangeEmail() {
  ...
  const queryClient = useQueryClient()
  const invalidate = () => queryClient.invalidateQueries(cacheKeys.users)
  const mutation = useMutation(() => api.patch(`${cacheKeys.users}/${user.id}`, { email }), {
    onSuccess: invalidate,
  })
...
function ChangeEpsilon() {
  ...
  const invalidate = () => queryClient.invalidateQueries(cacheKeys.users)
  const mutation = useMutation(
    () => api.patch(`${cacheKeys.users}/${user.id}`, { budget: parseFloat(budget) }),
    {
      onSuccess: invalidate,
    }
  )
...
function DeleteUser() {
  ...
  const mutation = useMutation(() => api.delete(`${cacheKeys.users}/${user.id}`), {
    onSuccess: invalidate,
  })
  ...
  return (
    <div className="p-8 space-y-6 rounded-md bg-blueGray-200">
      <header className="max-w-xl space-y-2">
        ...
        <p>
          PyGrid utilizes users and roles to appropriately permission data at a higher level. All
