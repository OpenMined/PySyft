"""Regenerate gemma_inference_ranges.obfuscated.py + its certificate.

Run from the package root:  uv run python examples/generate_ranges.py

This is the explicit-line-ranges variant of the example. Markers are the supported UX (see
gemma_inference.py / generate.py); this file drives the same model through the internal ``_run``
entry point with hand-counted ranges instead, exercising that path.
...
result = _run(
    path=EX / "gemma_inference_ranges.py",
    obfuscate=[
...
cert_path = EX / "gemma_inference_ranges.certificate.json"
