...
from syft.sync.connections.drive.gdrive_transport import SCOPES as GDRIVE_SCOPES
...
def download_from_gdrive(
    ...
    Args:
        ...
        token_path: Path to OAuth token JSON. Falls back to SYFT_TOKEN_PATH
...
def _build_service(token_path: str | Path | None = None):
    ...
    if not resolved:
        raise ValueError(
            "No token path provided. Set SYFT_TOKEN_PATH env var "
            "(was SYFTCLIENT_TOKEN_PATH before 0.10.0) or pass token_path explicitly."
        )
