...
def _resolve_dataset(
    ...
    if client is None and not in_job and not can_create_client:
        raise ValueError(
            ...
            f"    sy.resolve_dataset_file_path('{dataset_name}', client=client)\n\n"
...
def load_dataset_code(
    ...
    Examples:
        >>> import syft as sy
        >>> mod = sy.load_dataset_code("my_dataset.helpers", client=client)
        # → loads <dataset_dir>/helpers.py
        >>> mod = sy.load_dataset_code("my_dataset.utils.nested", client=client)
...
def _get_syft_info() -> dict[str, Any]:
    """Get syft specific information."""
    info: dict[str, Any] = {}

    try:
        import syft

        info["syft_version"] = getattr(syft, "__version__", "unknown")
    except ImportError:
        info["syft_version"] = "not installed"
    ...
    try:
        ...
        info["version_constants"] = {
            ...
            "MIN_SUPPORTED_SYFT_VERSION": MIN_SUPPORTED_SYFT_VERSION,
...
def _get_relevant_packages() -> dict[str, str | None]:
    """Get versions of packages relevant to syft (not full package list)."""
    # Only check packages relevant to debugging syft issues
    relevant_packages = [
        "syft",
        "syft-datasets",
        "syft-job",
...
def bug_report(
    ...
    Examples:
        >>> import syft as sy
        >>> sy.bug_report()  # Prints full report

        >>> report = sy.bug_report(as_dict=True)  # Get as dict
        >>> print(report["syft"]["syft_version"])
    """
    if not as_dict:
    ...
    print("\nTo get as dict: report = sy.bug_report(as_dict=True)")
