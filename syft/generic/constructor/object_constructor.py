...
from syft.generic.constructor.random import generate_cryptographically_secure_random_id
...
class ObjectConstructor(object):
    ...
    For example in the PyTorch framework we might wish to:
     ...
     - add additional functionality before/after th.Tensor() such as object registration (or memory
       re-use)

    However, as most deep learning frameworks are Python code wrapping C++, they often have very
    "locked down" __init__ and __new__ method definitions, which makes it difficult to modify their
    constructors in the conventional way. Furthermore, most frameworks (including Tensorflow,
    PyTorch, and NumPy), occasionally have multiple ways of constructing the same object
    (tensorflow.constant vs tensorflow.Tensor, and torch.Tensor vs torch.tensor) being two examples.
    Note that in each of these examples, one of the ways is the actual TYPE of tensor (th.Tensor)
    while the other is merely a method which can be used to create the tensor (th.tensor).

    As many constructors are difficult to overload (or perhaps have differing ways to overload
    them), we must use a more creative approach to augmenting constructor functionality. Instead of
    overloading __new__ or __init__, we create a series of 3 methods (pre_init, init, and post_init)
    within this ObjectConstructor class. Then, when you call __call__ on this class, it initializes
    the object using all three methods. Once we have defined this class, we then take a 2-step
    approach to *install* this class within an existing library.

    1) COPY: we copy the existing constructor to a "original_<name>" location. For example:

            th.original_Tensor = th.Tensor

    2) REPLACE: we then replace the original class path name with an instance of this constructor.
       For example:

            th.Tensor = TensorConstructor() # where TensorConstructor is an instance of ObjectConstructor

    This allows us to have a SINGLE way of implementing the following functionality for any object
    and installing it into the library (ensuring that all instances of that object use our
    constructor, even instances created from within the framework itself):
    ...
    Thus, if any object has it's functionality extended or overridden by PySyft, it should be
    created using an extension of this class.

    GARBAGE COLLECTION NOTES:

    There is a special case in codebases which wrap C++ functionality wherein an object can be
    created on the C++ side, a Python wrapper is created (calling our constructor above). However,
    some methods can destroy and then re-create the Python wrappr without destroying or re-creating
    the underlying tensor. This can be very tricky to deal with for a variety of reasons. However,
    we have observed that we can override this ability by:

    - caching the Python object somewhere so that the original python object doesn't get destroyed
      until it should be
    - intelligently remembering that this object has been created before when the constructor is
        called for the second time. This assume we can have stable "ID"s across creations of the
        tensor.

    This can be very tricky to implement but we have been able to make stable codebases with one or
    both of these approaches.
    """

    # This represents the name of the constructor this constructor is wrapping. Also, it sometimes
    # represents the type that this constructor will imitate so that isinstance(obj,
    # ObjectConstructor) will operate as if you instead called isinstance(obj,
    # original_constructor). This allows us to replace a framework constructor with our constructor.
    constructor_name: str = "String name of a constructor"
    ...
    def install_inside_library(self):
        ...
        if not isinstance(
            ...
            # If the original constructor is a class (not just a standalone func like tf.constant or
            # th.tensor)
    ...
    def install_id_attribute(self, original_constructor):
        ...
        if inspect.isclass(original_constructor) or self.constructor_produces_type is not None:
            ...
            # If this raises 'TypeError: type <> is not an acceptable base type' then it's a special
            # class of which Python cannot subtype for more on this, see discussion:
    ...
    def store_original_constructor(self):
        ...
        Since all instances of ObjectConstructor are overloading an existing constructor within a
        library, we must first copy th original constructor (called the the "original" constructor)
        to a consistent location, as determined by the 'gete_original_constructor_name' utility
        method.
        """
        ...
        if not hasattr(self.constructor_location, self.original_constructor_name):
            setattr(self.constructor_location, self.original_constructor_name, original_constructor)
    ...
    def pre_init(self, *args, **kwargs):
        ...
        If you need to create metadata needed for init or post_init, store such information
        within kwargs.
    ...
    def post_init(self, obj: object, *args, **kwargs):
        ...
        This method executes functionality which can only be run after an object has been
        initailized. It is particularly useful for logic which registers the created object into an
        appropriate registry. It is also useful for adding arbitrary attributes to the object after
        initialization.

        Args: *args (tuple): the arguments being used to initialize the object **kwargs (dict): the
            kwarguments eeing used to initialize the object Returns: out (SyftObject): returns the
            underlying syft object.
        """
    ...
    def original_constructor(self):
        """Return the original constructor for this method

        (i.e., the constructor the library had by default which this custom constructor overloaded.

        Note: try/except is used in this method instead of if/else because it's faster at runtime.
        """
    ...
    def __instancecheck__(cls, instance):
        ...
        constructor we also need this constructor to represent the type that it replaces, so that
        methods such as isinstance(my_tensor, th.Tensor) work correctly.

        Args: instance (object): an object of which we want to check the type against cls.
