...
class SimplePolynomial(nn.Module):
    ...
    def forward(self, x):
        ...
        for val_i in range(flattened.shape[0]):
            ...
            if (self.verbose):
                sys.stdout.write("\rNonlin - x^" + str(self.power) + " x " + str(self.scalar) + " - " + str(
                    (val_i / flattened.shape[0]) * 100)[0:4] + "%")
            out = val
            for _ in range(self.power - 1):
                out = out * val
            new_x.append((out * self.scalar).unsqueeze(0))
        if (self.verbose):
            sys.stdout.write("\rNonlin - x^" + str(self.power) + " x " + str(self.scalar) + " - 100%")
            print()
