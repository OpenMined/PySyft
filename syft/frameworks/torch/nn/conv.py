...
class Conv2d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        temp_init = Conv2d(in_channels=self.in_channels,
    ...
    def forward(self, data):
        ...
        expanded_data = data.unsqueeze(1).expand(batch_size,
                                                 self.out_channels,
                                                 1,
                                                 rows,
                                                 cols)

        expanded_model = self.weight.unsqueeze(0).expand(batch_size,
                                                         self.out_channels,
                                                         1,
                                                         self.kernel_size,
                                                         self.kernel_size)
        ...
        if self.has_bias:
            pred = pred + self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3).expand(batch_size,
                                                                                  self.out_channels,
                                                                                  rows - self.kernel_size + 1,
                                                                                  cols - self.kernel_size + 1)
