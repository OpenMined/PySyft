...
class FVTensor(AbstractTensor):
    def __init__(self, owner=None, id=None, tags=None, description=None):
        """Initializes a FVTensor, whose behaviour is to perform FV Homomorphic encryption
        applied on it.
        Args:
            owner: An optional BaseWorker object to specify the worker on which
                the tensor is located.
            id: An optional string or integer id of the FVTensor.
        """
        super().__init__(id=id, owner=owner, tags=tags, description=description)
        print("creating FVTensor")

    def keygen(self, plain_text):
        pass

    def encrypt(self, public_key):
        """This method will encrypt each value in the tensor using FV
        homomorphic encryption.
        Args:
            *public_key a public key created using
                syft.frameworks.torch.he.fv.keygen()
        """
    ...
    @staticmethod
    def simplify(worker: AbstractWorker, tensor: "FVTensor") -> tuple:
        """
        This function takes the attributes of a FVTensor and saves them in a tuple
        Args:
            tensor (FVTensor): a LogTensor
        Returns:
    ...
    @staticmethod
    def detail(worker: AbstractWorker, tensor_tuple: tuple) -> "FVTensor":
        """
        ...
        Returns:
            FVTensor: a LogTensor
        Examples:
            logtensor = detail(data)
        """
        obj_id, chain = tensor_tuple

        tensor = FVTensor(owner=worker, id=obj_id)
