...
class PolynomialTensor(NumpyTensor):
    ...
    def __init__(
        ...
        """
        super().__init__(numpy_tensor=polynomials, id=id, owner=owner, tags=tags, description=description)
    ...
    def sum(self, _self, dim):
        ...
        if (len(matrix.shape) == 2):
            ...
            if (dim == 0):
                ...
                print("Dim " + str(dim) + " out of range for matrix with shape " + str(matrix.shape))
...
def vsum(vector):
    ...
    return Polynomial(terms=additive_terms,
                      constant=additive_constant,
                      minimum_factor=p.minimum_factor).reduce()
