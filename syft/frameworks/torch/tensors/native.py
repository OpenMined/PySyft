...
class TorchTensor(AbstractTensor):
    ...
    def send(self, location):
        ...
        Args:
            location (....workers.BaseWorker): The BaseWorker object which you
                want to send this object to. Note that this is never actually
                the BaseWorker but instead a class which instantiates the
                BaseWorker abstraction.

        Returns:
            torch.Tensor[PointerTensor]: A pointer to self. Note that this
    ...
    def create_pointer(
        ...
        Args:
            location (....workers.BaseWorker): The BaseWorker object which
                points to the worker on which this pointer's object can be
                found. In nearly all cases, this is self.owner and so this
                attribute can usually be left blank. Very rarely you may know
                that you are about to move the Tensor to another worker so you
                can pre-initialize the .location attribute of the pointer to
                some other worker, but this is a rare exception.
            id_at_location (str or int): The id of the tensor being pointed to.
                Similar to location, this parameter is almost always self.id
                and so you can leave this parameter to None. The only exception
                is if you happen to know that the ID is going to be something
                different than self.id, but again this is very rare and most
                of the time, setting this means that you are probably doing
                something you shouldn't.
            register (bool): This parameter determines whether to register the
                new pointer that gets created. By default, this is set to false
                because (most of the time) a pointer is initialized in this way
                so that it can be sent to someone else (i.e., "Oh you need to
                point to my tensor? let me create a pointer and send it to you"
                ).Thus, when a pointer gets created, we want to skip being
                registered on the local worker because the pointer is about to
                be sent elsewhere. However, if you are initializing a pointer
                you intend to keep, then it is probably a good idea to register
                it, especially if there is any chance that someone else will
                initialize a pointer to your pointer.
            owner (....workers.BaseWorker): While location specifies where
                the pointer points to, this parameter specifies the worker on
                which the pointer is located. It is also where the pointer is
                registered if register is set to True.
            id (str or int): If you want to set the id of the pointer for any
                special reason, you can set it here. Otherwise, it will be set
                randomly.

        Returns:
            torch.Tensor[PointerTensor]: This method returns the pointer
            to self. Note that this object will likely be wrapped by a
