...
def decompose(tensor, field):
    ...
    field_to_dtype = torch.int64 if field > 2 ** 32 else torch.int32
    n_bits = round(math.log(field, 2))
    powers = torch.arange(n_bits, dtype=field_to_dtype)
...
def _random_common_value(max_value, *workers):
    ...
    in the form of a MultiPointerTensor
    """
    field_to_dtype = torch.int64 if max_value > 2 ** 32 else torch.int32
    pointer = (
...
def _shares_of_zero(size, field, dtype, crypto_provider, *workers):
    ...
    u = (
        torch.zeros(size, dtype=field_to_dtype)
        .send(workers[0])
...
def share_convert(a_sh):
    ...
    delta = ((delta > ((L - 1) // 2)) + (delta < -(L // 2))).type(torch_dtype)
    x = a_tilde_sh.get()
...
def division(x_sh, y_sh, bit_len_max=Q_BITS // 2):
    ...
    # Common Randomness
    w_sh = _shares_of_zero(bit_len_max, L, field_to_dtype, crypto_provider, alice, bob)
    s_sh = _shares_of_zero(1, L, field_to_dtype, crypto_provider, alice, bob)
    u_sh = _shares_of_zero(1, L, field_to_dtype, crypto_provider, alice, bob)
...
def maxpool(x_sh):
    ...
    # Common Randomness
    u_sh = _shares_of_zero(1, L, field_to_dtype, crypto_provider, alice, bob)
    v_sh = _shares_of_zero(1, L, field_to_dtype, crypto_provider, alice, bob)

    # 1)
    max_sh = x_sh[0]
    ind_sh = torch.tensor([0]).share(
        alice, bob, field=L, dtype=field_to_dtype, crypto_provider=crypto_provider, **no_wrap
...
def maxpool_deriv(x_sh):
    ...
    # Common Randomness
    U_sh = _shares_of_zero(n, L, field_to_dtype, crypto_provider, alice, bob)
    ...
    t = k_sh.get()
    k = t % n
    E_k = torch.zeros(n, dtype=field_to_torch_dtype)
    E_k[k] = 1
    E_sh = E_k.share(alice, bob, field=L, dtype=field_to_dtype, **no_wrap)
