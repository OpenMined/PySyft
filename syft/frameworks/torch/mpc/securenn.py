...
def decompose(tensor):
    ...
    # TODO: dtype int
    # field_to_torch_dtype = torch.int32 if field <= 2 ** 32 else torch.int64
    n_bits = Q_BITS
    powers = torch.arange(n_bits).long()
...
def _random_common_value(max_value, *workers):
    ...
    in the form of a MultiPointerTensor
    """
    # TODO: better way to handle with dtype
...
def msb(a_sh):
    ...
    u = _shares_of_zero(1, L, "long", crypto_provider, alice, bob)
...
def share_convert(a_sh):
    ...
    crypto_provider = a_sh.crypto_provider
    L = a_sh.field
    field_to_dtype = torch.int64 if L > 2 ** 32 else torch.int32
    # Common randomness
    eta_pp = _random_common_bit(*workers)
    # TODO: maybe revert below
...
def division(x_sh, y_sh, bit_len_max=Q_BITS // 2):
    ...
    # Common Randomness
    w_sh = _shares_of_zero(bit_len_max, L, "long", crypto_provider, alice, bob)
    s_sh = _shares_of_zero(1, L, "long", crypto_provider, alice, bob)
    u_sh = _shares_of_zero(1, L, "long", crypto_provider, alice, bob)
...
def maxpool(x_sh):
    ...
    # Common Randomness
    u_sh = _shares_of_zero(1, L, "long", crypto_provider, alice, bob)
    v_sh = _shares_of_zero(1, L, "long", crypto_provider, alice, bob)
...
def maxpool_deriv(x_sh):
    ...
    # Common Randomness
    U_sh = _shares_of_zero(n, L, "long", crypto_provider, alice, bob)
    # TODO: maybe revert below
    ...
    # 3)
    t = k_sh.get()
    # TODO: consider which modulo is to be used
    k = t % n
