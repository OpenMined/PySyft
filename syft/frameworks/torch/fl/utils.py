...
def add_model(dst_model, src_model,dst_no_data):
    ...
    with torch.no_grad():
        for name1, param1 in params1:
            if name1 in dict_params2:
                dict_params2[name1].set_(param1.data*train_data[src_model[i].location.id]['y'] + dict_params2[name1].data*dst_no_data)
    return dst_model
...
#     len(train_data[model.location.id]['y'])#number of one clients's data
    model_list = list(models.values())
    model = model_list[0]
    total_no_data=train_data[model.location.id]['y']
    for i in range(1, nr_models):
        model = add_model(model, model_list[i],total_no_data)
        total_no_data=total_no_data+train_data[model_list[i].location.id]['y']
    model = scale_model(model, 1.0 / total_no_data)
    return model
