...
def unwrap_args_with_function(args, return_tuple=False):
    ...
    args_hook_function = build_hook_for_args_child_with_rules(args, rule, return_tuple)
...
def build_wrap_reponse_with_function(response, wrap_type, wrap_args):
    ...
    # Build a function with this rule to efficiently replace syft tensors
    # (but not pointer) with their child in the args objects
    response_hook_function = build_response_hook_with_rule(response, rule, wrap_type, wrap_args)
...
def build_hook_for_args_child_with_rules(args, rules, return_tuple=False):
    """
...
def build_response_hook_with_rule(response, rules, wrap_type, wrap_args, return_tuple=False):
    """
    ...
    lambdas = [
        ...
        if not r  # if the rule is a number == 0.
        else build_response_hook_with_rule(
