...
class MPCRepo(object):
    ...
    def generate_multiplication_triple(self):
        ...
        c = a * b % Q
        return self.create_natural_with_shares(
            self.share(a)), self.create_natural_with_shares(
            self.share(b)), self.create_natural_with_shares(self.share(c))
    ...
    def mult(self, new_id, x, y, populate_to_another_party=False):
        ...
        delta = self.reconstruct(
            [self.ints[d.id], self.another_party[0].ints[d.id]])
        epsilon = self.reconstruct(
            [self.ints[e.id], self.another_party[0].ints[e.id]])
        ...
        result = self.add(new_id7, s.id, self.add(new_id6, t.id,
                                                  self.add_public(new_id5,
                                                                  c.id, r,
                                                                  True).id,
                                                  True).id, True)
    ...
    def mult_public(self, new_id, id1, scalar,
                    populate_to_another_party=False):
