...
def test_change_groups(client):
    result = client.put("/users/546313/groups", data={"groups": "new_groups"})
    assert result.status_code == 200
    assert result.get_json() == {"msg": "User groups was changed succesfully!"}
