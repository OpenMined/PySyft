...
def test_select_share(workers):
    ...
    alpha_1 = torch.tensor([[0, 1]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
    ones = torch.tensor([[1, 1]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
    twos = torch.tensor([[2, 2]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
...
def test_division(workers):
    ...
    tensorA = (
        torch.tensor([[0, 10, 10, 20, 20]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
    )
    tensorB = (
        torch.tensor([[1, 2, 3, 4, 5]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
    )
...
def test_maxpool(workers):
    ...
    tensorA = (
        torch.tensor([[0, 1, 8, 3, 4]]).share(alice, bob, charlie, crypto_provider=james, dtype="long").child
    )
