...
def test_RNNCell():
    ...
    assert torch.all(torch.lt(torch.abs(output_syft-output_torch), 1e-10))
...
def test_LSTMCell():
    ...
    assert torch.all(torch.lt(torch.abs(hidden_syft-hidden_torch), 1e-10))
    assert torch.all(torch.lt(torch.abs(cell_syft-cell_torch), 1e-10))
...
def test_LSTM():
    ...
    output_syft, (hidden_syft,cell_syft) = rnn_syft(test_input, (test_hidden_state,test_cell_state))
    output_torch, (hidden_torch,cell_torch) = rnn_torch(test_input, (test_hidden_state,test_cell_state))
