...
def test_evaluate(beta, workers):
    ...
    x_sh = x.share(*workers, protocol="falcon")
    ring_size = x_sh.ring_size
    shape = x_sh.shape

    if beta:
        beta_sh = ReplicatedSharingTensor.one_shares(workers, ring_size=ring_size, shape=shape)
    else:
        beta_sh = ReplicatedSharingTensor.zero_shares(workers, ring_size=ring_size, shape=shape)
