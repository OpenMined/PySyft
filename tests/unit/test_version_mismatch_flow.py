...
def _reinitialize_manager(
    ...
    """Create a new SyftboxManager on the same local path and mock Drive store.
...
def _simulate_continue_on_mismatch(manager, backing_store):
    ...
    with (
        ...
        patch(
            "syft_client.sync.login_utils.delete_remote_syftbox",
...
def test_version_mismatch_continues_and_repairs():
    ...
    old_personal_name = f"{SYFT_CLIENT_VERSION}#{do_manager.email}"
...
def test_login_writes_the_remote_version_file_too():
    ...
    """
    from syft_client.sync.login import _init_client_login
    ...
    assert conn.read_own_version_file().syft_client_version == SYFT_CLIENT_VERSION
    ...
    with (
        ...
        assert (
            new_conn.read_own_version_file().syft_client_version == SYFT_CLIENT_VERSION
        )

        _init_client_login(upgraded, sync=False, load_peers=False)
...
def test_the_mismatch_prompt_does_not_return_after_a_login():
    ...
    with (
        ...
        patch("syft_client.sync.version.version_info.SYFT_CLIENT_VERSION", NEW_VERSION),
        patch("syft_client.sync.login_utils.SYFT_CLIENT_VERSION", NEW_VERSION),
        patch("syft_client.sync.login_utils._resolve_email", return_value=email),
        patch("syft_client.sync.login_utils._resolve_token_path", return_value=None),
        patch(
            "syft_client.sync.login_utils._get_default_syftbox_path",
            return_value=syftbox_folder,
        ),
        patch(
            "syft_client.sync.login_utils._prompt_mismatch", return_value="1"
        ) as mock_prompt,
    ):
        from syft_client.sync.login_utils import (
            handle_potential_version_mismatches_on_login,
