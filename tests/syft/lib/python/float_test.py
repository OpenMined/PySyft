...
class F:
    def __float__(self) -> OtherFloatSubclass:
...
def test_non_numeric_input_types() -> None:
    ...
    for f in factories:
        x = f(b" 3.14  ")  # type: ignore
        assert Float(x) == 3.14
        with pytest.raises(ValueError):
            Float(f(b"A" * 0x10))  # type: ignore
...
def test_Float_containment() -> None:
    ...
    for f in Floats:
        ...
        assert f in {f}
        assert f in {f: None}
        assert [f].count(f), 1 == f"[].count('{f!r}') != 1"  # type: ignore
        assert f in Floats
...
# fixes mypy complaining about calling pow
def call_pow(x: Any, y: Any) -> Any:
    return pow(x, y)
...
def test_Float_pow() -> None:
    ...
    for pow_op in call_pow, operator.pow:
        _ = pow_op(-INF, NAN)
...
def test_inf_nan() -> None:
    ...
    with pytest.raises(ValueError):
        round(NAN)
    with pytest.raises(TypeError):
        round(INF, 0.0)  # type: ignore
    with pytest.raises(TypeError):
        round(-INF, 1.0)  # type: ignore
    with pytest.raises(TypeError):
        round(NAN("ceci n'est pas un integer"))
    with pytest.raises(TypeError):
        round(-0.0, 1j)  # type: ignore
