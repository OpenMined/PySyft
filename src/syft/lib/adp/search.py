...
def create_searchable_function_from_polynomial(poly:Basic, symbol2index:dict) -> Callable:
...
def max_lipschitz_via_jacobian(
    ...
    if try_hessian_shortcut:
        ...
        if len(solve(np.sum(h ** 2), *[x.poly for x in input_scalars], dict=True)) == 0:
            ...
            search_fun = create_searchable_function_from_polynomial(poly=neg_l2_j, symbol2index=s2i)
