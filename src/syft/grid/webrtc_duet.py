...
class Duet(DomainClient):
    ...
    @syft_decorator(typechecking=True)
    async def push(self, route: str) -> None:
    ...
    @syft_decorator(typechecking=True)
    async def pull(self, route: str) -> None:
        try:
            while self._available:
                ...
                # send it to the signaling server.
                _response = self.network_client.send_immediate_msg_with_reply(
                    msg=msg, route=route
                )
    ...
    def send_offer(self, address: Address) -> None:
        ...
        signaling_offer = SignalingOfferMessage(
            address=address,  # Target's address
            payload=payload,  # Offer Payload
            target_metadata=self.node.get_metadata_for_client(),  # Own Node Metadata
            reply_to=self.node.address,  # Own Node Address
        )
    ...
    async def _send_answer(self, msg: SignalingOfferMessage) -> None:
        ...
        signaling_answer = SignalingAnswerMessage(
            address=msg.reply_to,  # Remote Node Address
    ...
    def _update_availability(self) -> bool:
        ...
        """
        return (
            not self._pull_msg_queue.empty() and self.connection.peer_connection != None
        )
